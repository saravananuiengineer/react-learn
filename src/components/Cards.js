const Cards = (props) => {
   /* const obj ={
    ...
    return (
        <div className="max-w-sm cards p-6 bg-white w-[250px border-gray-200 rounded-zl shadow-sm transition-all duration-300 ease-in-out cursor-pointer hover:bg-blue-50 hover:shadow-xl hover:border-blue-200 hover:-translate-y-1">
            <img  src={image.url} className="rounded-[8px] w-full h-48 object-cover"/>
            <div className="row-card flex justify-between">
...
export const promotedCard = (Cards)=>{
    return (props)=> {
        return(
            <div>
                <span className="absolute p-2 ml-2 bg-black rounded-lg text-white">promoted</span>
                <Cards {...props}/>
            </div>
        );
    }
